...
app.get("/getNote/:noteId", express.json(), async (req, res) => {
  ...
  // Retrieve aall notes belonging to the user
app.get("/getAllNotes/", express.json(), async (req, res) => {
  try {
    ...
    jwt.verify(token, "secret-key", async (err, decoded) => {
      ...
      var myCursor = collection.find({ username: decoded.username });

      const data = await myCursor.toArray();
...
app.delete("/deleteNote/:noteId", express.json(), async (req, res) => {
  try {
    ...
    const noteId = req.params.noteId;
    if (!ObjectId.isValid(noteId)) {
      return res.status(400).json({ error: "Bad request in relation "
      + "to the :noteId URL parameter" });
    }
    ...
    jwt.verify(token, "secret-key", async (err, decoded) => {
      ...
      if (!data) {
        return res
          .status(404)
          .json({ error: "Note with ID " + noteId + " belonging " 
          + "to the user not found" });
      }
      ...
      res.json({ response: 'Document with ID {' + noteId
      + '} properly deleted.' });
    });
  } catch (error) {
    res.status(500).json({ error: error.message });
...
app.patch("/patchNote/:noteId", express.json(), async (req, res) => {
  try {
    ...
    if (!ObjectId.isValid(noteId)) {
      return res.status(400).json({ error: "Bad request in relation "
      + "to the :noteId URL parameter, or the body params" });
    }
    const { title, content } = req.body;
    if (!title && !content) {
      return res.status(400).json({ error: "Bad request in relation "
    ...
    jwt.verify(token, "secret-key", async (err, decoded) => {
      ...
      if (content) {
        updateDocument.$set = { ...updateDocument.$set, content: content };
      }
